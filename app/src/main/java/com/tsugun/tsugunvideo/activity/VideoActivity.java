...
import com.tsugun.tsugunvideo.view.TsuGunVideo;

import java.lang.ref.WeakReference;

public class VideoActivity extends AppCompatActivity {

    private static final String TAG = "TsuGunTestTAG";
    public static final String INTENT_VIDEO_MESSAGE = "com.tsugun.tsugunvideo.VideoActivity.info";
    // handler更新标识
    ...
    public VideoUIHandler mVideoUIHandler; // 自定义UIHandler
    private AudioManager mAudioManager; // 音频管理器
    private Drawable mBarThumb;
    private MediaFileInfo mMediaFileInfo; // 视频文件信息
    private MediaFileFinder mInfoFinder; // 数据获取器

    private TextView mInfoName; // 视频名称
    private TextView mInfoDuration; // 视频总时间
    private TextView mInfoSize; // 视频文件大小
    private TextView mInfoPath; // 视频文件路径
    ...
    private void initView() {
        ...
        mInfoName = (TextView) findViewById(R.id.id_info_name);
        mInfoDuration = (TextView) findViewById(R.id.id_info_duration);
        mInfoSize = (TextView) findViewById(R.id.id_info_size);
        mInfoPath = (TextView) findViewById(R.id.id_info_path);
    }
    ...
    private void initData() {
        ...
        mInfoFinder = new BaseDBHelper(VideoActivity.this, DB_TUSGUN, 1);
        mAudioManager = (AudioManager) getSystemService(Context.AUDIO_SERVICE);
        mBrightness = SettingsUtil.getSystemBrightness(this);
     ...
     * 设置视频信息
     */
    private void setVideoMsg() {
        String videoUrl = getIntent().getStringExtra(INTENT_VIDEO_MESSAGE);
        ...
        mInfoDuration.setText(time);
        mInfoSize.setText(size);
        mInfoPath.setText(path);
    }
    ...
    private View.OnTouchListener mVideoTouchListener = new View.OnTouchListener() {
        ...
        private void setBright() {
            ...
            if (isAddValue) {
                // 增加亮度
                SettingsUtil.setSystemBrightness(VideoActivity.this,
        ...
        private int checkTouchOrientation(MotionEvent event) {
            if (Math.abs(mMoveX) > Math.abs(mMoveY)) {
                ...
                if (event.getX() < metrics.widthPixels / 2) {
                    // 判断为左侧纵向滑动,控制亮度
...
//            mTsuGunVideo.setRotation(mVideoPathList.get(mPathPosition).getRotation());
     ...
     * 全屏按钮点击事件
     */
    public void onFullButtonClick(View view) {
        if (mMediaFileInfo != null) {
    ...
    private void onChangeScreenState() {
        ...
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            // 横屏
            mFullBtn.setBackgroundResource(R.drawable.ic_vector_control_not_screen);

            LayoutParams layoutParams = mTsuGunVideo.getLayoutParams();
            layoutParams.height = LayoutParams.MATCH_PARENT;
            mTsuGunVideo.setLayoutParams(layoutParams);

            getWindow().clearFlags(WindowManager.LayoutParams.FLAG_FORCE_NOT_FULLSCREEN);
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
        } else {
            // 竖屏
            mFullBtn.setBackgroundResource(R.drawable.ic_vector_control_full_screen);

            LayoutParams layoutParams = mTsuGunVideo.getLayoutParams();
            // px和dp的相互转化
            layoutParams.height = (int) ConverterUtil.dipToPx(VideoActivity.this, 220);
            mTsuGunVideo.setLayoutParams(layoutParams);
    ...
    protected void onPause() {
        ...
        isVideoPlay = false;
        if (mTsuGunVideo != null) {
            mCurrent = mTsuGunVideo.getCurrentPosition();
            if (mTsuGunVideo.isPlaying()) {
                mTsuGunVideo.pause();
                isVideoPlay = true;
            }
        }
        Log.i(TAG, "onPause: " + mCurrent);
    }
    ...
    private static class VideoUIHandler extends Handler {
        ...
        VideoUIHandler(VideoActivity activity) {
        ...
        public void handleMessage(Message msg) {
            ...
            VideoActivity activity = activityWeakReference.get();
            if (activity != null) {
                switch (msg.what) {
                    case PROGRESS_BAR_UPDATE:
                        if (!activity.isProgressTouch) {
                            int duration = activity.mTsuGunVideo.getDuration();
                            int current = activity.mTsuGunVideo.getCurrentPosition();
